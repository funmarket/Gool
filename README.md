...
The repository deliberately does **not** contain a hand-written fake initial migration. Before the first GitHub/deployment baseline is accepted, create the migration from the checked-in Prisma schema against a fresh PostgreSQL database:
...
Commit the generated `packages/database/prisma/migrations/<timestamp>_init/` directory. For later production deployments use only `npm run db:migrate:deploy`. Never use `prisma db push` in production and never edit a migration after it has shipped.
...
- application source gate: previously passed on the networked Windows development machine; rerun after any canonical-source change.
- initial Prisma migration: **not generated yet**; requires PostgreSQL.
- dependency security audit: **must be green before GitHub**; do not use `npm audit fix --force`.

Do not call the repository deployment-ready until `npm run release:check` and `npm run deploy:preflight` both pass with the real database configuration.
