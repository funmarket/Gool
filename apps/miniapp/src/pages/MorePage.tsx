import { HelpCircle, Settings, Share2, Star, UserRound, ShieldCheck } from 'lucide-react';
...
export function MorePage() {
  ...
  return (
    <div className="page-shell">
      <div className="section-kicker">GOOL</div>
      ...
      <div className="grid gap-3">
        ...
        {active && ['OWNER', 'ADMIN'].includes(active.role) && (
          <ActionRow icon={<ShieldCheck />} title="Admin" onClick={() => navigate('/admin')} />
        )}
        <ActionRow icon={<Settings />} title="Settings" onClick={() => navigate('/settings')} />
        <ActionRow icon={<Star />} title="Rate GOOL" />
        <ActionRow icon={<HelpCircle />} title="Support" />
        <ActionRow icon={<Share2 />} title="Share Mini App" />
      </div>
    </div>
  );
}
